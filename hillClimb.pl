...
sub hillClimbMin {
	...
	$movedInLastIteration = 1;	#1 if the algorithm has moved to a different point when previously iterating through each parameter (independent variable)'s dimension space. 0 if has not moved
...
print "\n\n3 DIMENSIONAL OPTIMIZATION\n";
my @inputs = hillClimbMax(\&f2, $tol, $x0, $x1);
my $val = f2(@inputs);
print "Min of function f2 (starting at ($x0, $x1), to within tolerance $tol, initial step size = 1) is:\n f(", join(', ', @inputs), ") == $val\n";
=cut
...
#@parameterList = hillClimbMax(\&f1 , 0.1, 1);

#print "\n\n2 DIMENSIONAL OPTIMIZATION\n";
#@inputs = hillClimbMax(\&g, $tol, $x0);
#$val = g(@inputs);
#print "Min of function g (starting at ($x0), to within tolerance $tol, initial step size = 1) is:\n f(", join(', ', @inputs), ") == $val\n";
